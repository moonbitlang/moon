...
use std::{
    collections::HashMap,
    ffi::CString,
    fs::{self, metadata, File, OpenOptions, Permissions, ReadDir},
    io::{IsTerminal, Read, Write},
    os::{
        fd::AsRawFd,
        unix::{
            self,
            fs::{FileExt, FileTypeExt, MetadataExt, OpenOptionsExt, PermissionsExt},
        },
    },
    path::Path,
...
impl FdTable {
    ...
    fn get(&self, fd: i32) -> Result<&File, String> {
        match self.map.get(&fd) {
            None => Err(format!("invalid file descripter: {}", fd)),
...
// open : JSString as Path, Number as Flags, Number as PermissionMode -> FileDescripter
fn open(
    scope: &mut v8::HandleScope,
    args: v8::FunctionCallbackArguments,
    mut ret: v8::ReturnValue,
...
// close : FileDescripter -> undefined
fn close(
    scope: &mut v8::HandleScope,
    args: v8::FunctionCallbackArguments,
    mut ret: v8::ReturnValue,
...
fn write(
    ...
    let bytes: &[u8] =
        unsafe { std::slice::from_raw_parts(raw_data as *mut u8, buf_length) };
...
// JSString as Path, F64 as AccessTime, F64 as ModifyTime -> undefined
fn utimes(
    scope: &mut v8::HandleScope,
    args: v8::FunctionCallbackArguments,
    mut ret: v8::ReturnValue,
...
// read_dir: JSString as Path -> Array<String>
fn read_dir(
    scope: &mut v8::HandleScope,
    args: v8::FunctionCallbackArguments,
    mut ret: v8::ReturnValue,
...
fn decode_utf8(
    ...
    let copyed = jsbytes.copy_contents(&mut bytes);
    assert!(copyed == size);
