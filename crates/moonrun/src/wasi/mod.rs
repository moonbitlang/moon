...
//! Backend-neutral WASIp1 Host state and operations.

use crate::run_termination::{RunTermination, TerminationRequest};
use crate::runtime::{Env, Runtime, Stdio, StdioStream};
...
pub(crate) type WasiErrno = i32;
pub(crate) type WasiResult<T> = Result<T, WasiErrno>;
...
pub(crate) const WASI_ERRNO_FAULT: WasiErrno = 21;
pub(crate) const WASI_ERRNO_INVAL: WasiErrno = 28;
...
/// Per-Run backend-neutral WASIp1 state.
///
/// Engine adapters borrow current Guest Memory for one call and invoke the
/// operations below; descriptor, capability, path, environment, and stdio
/// semantics live here rather than in either engine backend.
...
impl WasiContext {
    pub(crate) fn new(
        ...
        let preopen_dir_host_path = runtime
            .working_directory()
            .current_dir()
            .unwrap_or_else(|_| PathBuf::from("."));
        let preopen_dir_real_path = fs::canonicalize(&preopen_dir_host_path)
            .unwrap_or_else(|_| preopen_dir_host_path.clone());
        Self {
            argv: build_argv(wasm_file_name, args),
            environment: Arc::clone(runtime.environment()),
            stdio: Arc::clone(runtime.stdio()),
...
pub(crate) fn result_to_errno(result: WasiResult<()>) -> WasiErrno {
    match result {
...
pub(crate) fn random_get_impl(memory: &mut [u8], buffer: u32, length: u32) -> WasiResult<()> {
...
pub(crate) fn fd_close_impl(context: &WasiContext, fd: i32) -> WasiResult<()> {
...
pub(crate) fn path_open_impl(
    ...
    let path_len = usize::try_from(path_len).map_err(|_| WASI_ERRNO_INVAL)?;
    ...
    if (rights_base & !parent_inheriting_rights) != 0
        || (rights_inheriting & !parent_inheriting_rights) != 0
    {
        return Err(WASI_ERRNO_NOTCAPABLE);
    }

    let path = read_path_from_memory(memory, path_ptr, path_len)?;
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn path_readlink_impl(
    context: &WasiContext,
    memory: &mut [u8],
    dirfd: i32,
    path_ptr: u32,
    ...
    let path_len = usize::try_from(path_len).map_err(|_| WASI_ERRNO_INVAL)?;
    let buf_len = usize::try_from(buf_len).map_err(|_| WASI_ERRNO_INVAL)?;
...
pub(crate) fn path_create_directory_impl(
    context: &WasiContext,
    memory: &mut [u8],
    dirfd: i32,
    path_ptr: u32,
...
pub(crate) fn path_rename_impl(
    ...
    let old_path_len = usize::try_from(old_path_len).map_err(|_| WASI_ERRNO_INVAL)?;
    let new_path_len = usize::try_from(new_path_len).map_err(|_| WASI_ERRNO_INVAL)?;
    ...
    let old_path = read_path_from_memory(memory, old_path_ptr, old_path_len)?;
    let new_path = read_path_from_memory(memory, new_path_ptr, new_path_len)?;
...
pub(crate) fn path_remove_directory_impl(
    context: &WasiContext,
    memory: &mut [u8],
    dirfd: i32,
    path_ptr: u32,
...
pub(crate) fn path_unlink_file_impl(
    context: &WasiContext,
    memory: &mut [u8],
    dirfd: i32,
    path_ptr: u32,
...
fn poll_oneoff_impl(
    ...
    let nsubscriptions = usize::try_from(nsubscriptions).map_err(|_| WASI_ERRNO_INVAL)?;
    let mut subscriptions = parse_poll_subscriptions(memory, in_ptr, nsubscriptions)?;
...
pub(crate) fn fd_prestat_get_impl(
    context: &WasiContext,
    memory: &mut [u8],
    fd: i32,
    prestat_ptr: u32,
...
pub(crate) fn fd_prestat_dir_name_impl(
    context: &WasiContext,
    memory: &mut [u8],
    fd: i32,
    path_ptr: u32,
...
pub(crate) fn args_sizes_get_impl(
    context: &WasiContext,
    memory: &mut [u8],
    argc_ptr: u32,
...
pub(crate) fn environ_sizes_get_impl(
    context: &WasiContext,
    memory: &mut [u8],
    environc_ptr: u32,
...
pub(crate) fn proc_exit_impl(context: &WasiContext, code: u32) {
        ...
        WASI_FD_STDOUT => {
            context.stdio.with_stdout(|stdout| -> WasiResult<()> {
                for index in 0..iovs_len {
                    ...
                    let bytes = checked_range(memory, buf_offset, len)?;
