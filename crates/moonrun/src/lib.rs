...
/// Applies the legacy command-line process semantics for a run outcome.
///
/// Library callers should inspect [`RunOutcome`] instead. This function exists
/// only so the `moonrun` binary can remain a thin adapter without duplicating
/// platform-specific signal termination code.
#[doc(hidden)]
pub fn apply_cli_outcome(outcome: RunOutcome) -> anyhow::Result<()> {
    match outcome {
...
fn initialize_v8(config: &RuntimeConfig) -> anyhow::Result<()> {
...
fn wasm_mode(
    ...
    if let Some(termination) = termination {
        return Ok(match termination {
            run_termination::RunTermination::Exit(code) => RunOutcome::Exited(code),
            run_termination::RunTermination::KilledBySignal(signal) => {
                RunOutcome::KilledBySignal(signal)
            }
        });
    }
