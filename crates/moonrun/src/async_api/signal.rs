...
pub(super) fn set_global_cancellation_signals(
    ...
    let signals = signal::set_global_cancellation_signals(&all_signals, &signals);
    context.host.set_cancellation_signals(&all_signals, &signals)
}
...
pub(super) fn set_console_control_handler(
    ...
    let active = signal::set_console_control_handler(add != 0);
    context.host.set_signal_delivery_active(active)
}
}
