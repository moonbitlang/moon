...
"#,
        ...
        // Exercise normalization independently of compiler support for repeated
        // blocks in moon.pkg.
        ...
        if duplicate {
            result.stderr_eq(
                "Warning: Duplicate import of package `example/import_blocks/a` in `import`.\n",
            );
        } else {
            result.stderr_eq("");
        }
...
fn test_packages_json_full_check_and_focused_check_behavior() {
    ...
    let packages_selector: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&pkg_json).unwrap()).unwrap();
    assert_eq!(
        packages_selector,
        serde_json::json!({
            "backend": "wasm-gc",
