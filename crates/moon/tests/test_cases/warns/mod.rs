...
fn test_warn_list_dry_run() {
    ...
    check(
        ...
        expect![[r#"
            moonc build-package ./lib1/hello.mbt -w -1 -o ./_build/wasm-gc/debug/build/lib1/lib1.core -pkg username/hello/lib1 -std-path '$MOON_HOME/lib/core/target/wasm-gc/release/bundle' -i '$MOON_HOME/lib/core/target/wasm-gc/release/bundle/prelude/prelude.mi:prelude' -pkg-sources username/hello/lib1:./lib1 -target wasm-gc -g -O0 -source-map -workspace-path . -all-pkgs ./_build/wasm-gc/debug/build/all_pkgs.json
...
fn test_deny_warn() {
    ...
    check(
        ...
        expect![[r#"
            failed: moonc build-package -error-format json -w @a $ROOT/lib/hello.mbt -o $ROOT/_build/wasm-gc/debug/build/lib/lib.core -pkg username/hello/lib -std-path $MOON_HOME/lib/core/target/wasm-gc/release/bundle -i $MOON_HOME/lib/core/target/wasm-gc/release/bundle/prelude/prelude.mi:prelude -pkg-sources username/hello/lib:$ROOT/lib -target wasm-gc -g -O0 -source-map -workspace-path $ROOT -all-pkgs $ROOT/_build/wasm-gc/debug/build/all_pkgs.json
        "#]],
    );
}
