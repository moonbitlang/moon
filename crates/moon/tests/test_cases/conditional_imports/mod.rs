...
#[test]
fn test_package_graph_export_selects_backend() {
    let dir = TestDir::new("conditional_imports/fixture");
    std::fs::write(
...
#cfg(target = "native")
import { "example/conditional/native" @conditional_native_impl }
#cfg(target = "js")
import { "example/conditional/portable" @conditional_js_impl }
"#,
    )
    .unwrap();

    for (backend, active_alias, inactive_alias) in [
        ("native", "conditional_native_impl", "conditional_js_impl"),
        ("js", "conditional_js_impl", "conditional_native_impl"),
    ] {
        moon_cmd(&dir)
            .args([
                "-Z",
                "rr_export_package_graph",
                "check",
                "app",
                "--target",
                backend,
        ...
        let graph = std::fs::read_to_string(dir.join("_build/package_graph.dot")).unwrap();
        assert!(
            graph.contains(&format!("label=\"{active_alias}\"")),
            "{graph}"
        );
        assert!(
            !graph.contains(&format!("label=\"{inactive_alias}\"")),
            "{graph}"
        );
    }
}
