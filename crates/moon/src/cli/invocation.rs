    ...
    match command {
        ...
        command => {
            let output = if matches!(
                &command,
                MoonBuildSubcommands::Check(cmd) if cmd.json
            ) || matches!(
                &command,
                MoonBuildSubcommands::Search(cmd) if cmd.json
            ) || matches!(
                &command,
                MoonBuildSubcommands::Tree(cmd) if cmd.json
            ) {
                OutputFormat::Json
            } else {
                OutputFormat::Human
