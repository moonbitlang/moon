...
/// Components extracted from browser tree URLs for hosted git repositories.
...
pub(super) fn parse_hosted_git_tree_source(source: &str) -> Option<HostedGitTreeSource> {
    ...
    let path_in_repo = (after_tree.len() > 1).then(|| after_tree[1..].join("/"));

    Some(HostedGitTreeSource {
        git_url: format!("{scheme}://{host}/{owner}/{repo}"),
        branch: Some(after_tree[0].to_string()),
        path_in_repo,
    })
}
...
pub(super) fn build_registry_native_executable_to(
    ...
    let package_dirs = cli.source_tgt_dir.source_root_package_dirs(source.path())?;
    build_native_executable_to(
        &cli,
        module_name,
        source.path(),
        package_dirs,
...
mod tests {
    ...
    #[test]
    fn pinned_registry_version_uses_existing_index_entry() {
    ...
    #[test]
    fn pinned_registry_version_updates_missing_index_entry() {
        let updated = Cell::new(false);
        ensure_registry_version_available(
            &"testuser/runner".parse().unwrap(),
            &"1.2.3".parse().unwrap(),
