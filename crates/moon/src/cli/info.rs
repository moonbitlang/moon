...
pub fn run_info_rr(cli: UniversalFlags, cmd: InfoSubcommand) -> anyhow::Result<i32> {
    ...
    let (_build_meta, build_graph) = rr_build::plan_build(
        ...
        &source_dir,
        &target_dir,
        Box::new(move |r, m| calc_user_intent(package_filter.clone(), path_filter.clone(), r, m)),
    )?;
...
fn calc_user_intent(
    ...
    let intents = if let Some(path) = path_filter {
        ...
        let matched = match_packages_by_name_rr(resolve_output, main_modules, filter.as_str());
        if matched.is_empty() {
            bail!(
