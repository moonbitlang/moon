...
impl<'a> BuildPlanConstructor<'a> {
        ...
        if target.kind == TargetKind::Source {
            if let Some(vpkg) = &pkg.raw.virtual_pkg {
                if vpkg.has_default {
                    ...
                    unreachable!("A virtual package without default implementation should have a build target info");
                }
    ...
    fn dfs_link_core_sources(
        ...
        // Implement a manual iterative DFS that emits nodes in post-order
        // without allocating a separate children list for each frame.
        ...
        while let Some((node, expanded)) = stack.pop() {
            if !expanded {
                ...
                //
                // If the virtual package is overridden
            ...
            // TODO: Virtual package overrides
    ...
    pub(super) fn build_parse_mbti(
        ...
        for dep in self.input.pkg_rel.dep_graph.neighbors_directed(
            ...
            // TODO: We don't know which build node we will be relying on!
            // Coalesce `Check` to `Build` if both are present.
            self.need_mi_of_dep(node, dep, true);
        }
