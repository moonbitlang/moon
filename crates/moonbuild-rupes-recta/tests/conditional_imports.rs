...
use moonutil::{
    ...
    project::WorkspaceEnv,
    resolution::{DirSyncResult, ModuleDependencyGraph, ModuleSource},
    target::TargetBackend::{self, Js, Native},
    user_log::UserLog,
};

fn resolve(packages: &[(&str, &str)]) -> (tempfile::TempDir, ResolvedProject) {
    let (dir, discovered) = discover(packages);
    let resolved = discovered
        .resolve_packages(TargetBackend::all(), &UserLog::new(log::LevelFilter::Error))
        .unwrap();
    (dir, resolved)
}

fn discover(packages: &[(&str, &str)]) -> (tempfile::TempDir, DiscoveredProject) {
    ...
    let (module_graph, mid) = ModuleDependencyGraph::only_one_module(source, module);
    let mut module_dirs = DirSyncResult::new();
    module_dirs.insert(mid, dir.path().to_owned());
    let log = UserLog::new(log::LevelFilter::Error);
    let config =
        ProjectPreparationConfig::new_with_load_defaults(false, true, false, WorkspaceEnv::Auto);
    let discovered = discover_synced_project(&config, (module_graph, module_dirs), &log).unwrap();
    (dir, discovered)
}
...
fn resolved_project_already_contains_each_backends_graph() {
    ...
    for &backend in TargetBackend::all() {
        assert_eq!(
            resolved.package_relations[&backend]
                .dep_graph
...
fn target(resolved: &ResolvedProject, name: &str, kind: TargetKind) -> BuildTarget {
    resolved
        .pkg_dirs
...
fn imports(
    ...
    let mut imports = resolved.package_relations[&backend]
        .dep_graph
...
"#,
    ...
    for &backend in TargetBackend::all() {
        let supported = &resolved.package_relations[&backend].realizable_supported_targets
...
"#,
    ...
    let resolved = discovered
        .clone()
        .resolve_packages(&[Native, Native], &log)
        .unwrap();
    assert_eq!(
        resolved
            .package_relations
    ...
    for backends in [&[Js][..], &[Native, Js], &[Js, Native]] {
        let error = discovered
            .clone()
            .resolve_packages(backends, &log)
            .unwrap_err();
        assert!(
            matches!(error, ProjectPreparationError::PackageResolutionError(source)
            if matches!(*source, moonbuild_rupes_recta::pkg_solve::PackageResolutionError::ImportNotFound { ref import, .. }
...
fn overlapping_conditional_imports_still_report_errors() {
    ...
    for (app, other) in [
        ...
        let (_dir, discovered) = discover(&[("app", app), ("other", other), ("third", "")]);
        assert!(
            discovered
                .clone()
                .resolve_packages(&[Native], &log)
                .is_err()
        );
        assert!(discovered.resolve_packages(&[Js], &log).is_ok());
    }
}
...
"#,
    ...
    let resolved = discovered
        .resolve_packages(&[Native, Js], &UserLog::new(log::LevelFilter::Error))
        .unwrap();
    let app = target(&resolved, "app", TargetKind::Source);
    ...
    for backend in [Native, Js] {
        let relation = &resolved.package_relations[&backend];
