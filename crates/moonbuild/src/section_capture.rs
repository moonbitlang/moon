...
use tokio::{process::ChildStdout, sync::mpsc};
...
impl<'a> SectionCapture<'a> {
    ...
    pub async fn feed_line(&mut self, line: &str) -> Option<LineCaptured> {
        if line.trim_end().ends_with(self.begin_delimiter) {
            self.found_begin = true;
            self.found_end = false;
            if self.include_delimiters {
                self.sender.send(line.to_string()).await.unwrap();
            }
...
pub async fn handle_stdout<P: FnMut(&str)>(
    stdout: &mut tokio::io::BufReader<ChildStdout>,
    mut captures: Vec< SectionCapture<'_>>,
    mut print: P,
) -> anyhow::Result<()> {
    ...
    loop {
        buf.clear();
        let n = tokio::io::AsyncBufReadExt::read_line(stdout, &mut buf).await?;
        // let n = stdout.read_line(&mut buf)?;
        if n == 0 {
            break;
        }
        let capture_status =
            futures::future::join_all(captures.iter_mut().map(|capture| capture.feed_line(&buf)))
                .await
                .into_iter()
    ...
    captures.into_iter().for_each(|it| {
        drop(it);
    });
