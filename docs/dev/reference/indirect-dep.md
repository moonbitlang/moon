  ...
  "packages": [
    {
      ...
      "rel": "builtin",
      "artifact": "/path/to/_build/wasm-gc/release/build/moonbitlang/core/builtin/builtin.mi"
    },
    {
      "root": "username/mymodule",
      "rel": "utils",
      "artifact": "/path/to/_build/wasm-gc/release/build/username/mymodule/utils/utils.mi"
    }
  ]
}
```
...
`all_pkgs.json` is regenerated discovery metadata, not a compiler output
published to the action/output store. The compiler uses entries named by
imported interfaces. During single-file dependency preparation, those packages
already participate transitively through producer action digests and logical
products, so the local action identity does not hash the complete shared
metadata file as an additional external input.
