...
- **Explicit formatter scripts**. For `moon fmt script.mbtx`, script input and output paths are relative to the invocation directory, including parent and sibling directories. Paths on different Windows drives remain absolute. Package plans retain project-relative paths when combined with scripts.
...
- **`moon deprecate --dry-run`**. Moon fetches the module's currently published versions from the configured registry and prints the intended deprecation or restoration for each release, newest first, on stdout. It does not load publishing credentials or start the backend that modifies registry state. Failed registry lookups fail the preview. `--quiet` does not hide this preview.
