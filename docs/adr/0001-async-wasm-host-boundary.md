...
- Keep guest termination scoped to one wasm run. An import such as `runtime/exit` or `runtime/terminate_process_by_signal` records a per-run termination outcome and interrupts guest execution; it must not call `exit`, `raise`, or `ExitProcess` from inside the runtime adapter. After guest and host state have been torn down, the outer CLI adapter may translate that outcome into the standalone `moonrun` process exit or signal status. Future Wasm runtime adapters must surface the same outcome instead of terminating their embedding runner.
